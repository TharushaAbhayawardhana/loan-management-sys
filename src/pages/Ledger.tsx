...
export function Ledger() {
  ...
  const loanById = useMemo(() => new Map((data?.loans ?? []).map((l) => [l.id, l])), [data]);
  ...
  if (!data) {
    return <div className="animate-pulse text-sm text-[var(--color-ink-faint)]">Loading ledger…</div>;
  }
  ...
  const handleDelete = async (payment: PaymentLedger) => {
    ...
    await db.paymentLedger.delete(payment.id);
  };
